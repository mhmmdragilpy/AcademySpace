...
export const getReservationStats = catchAsync(async (req, res) => {
    ...
    const approvedReservationsResult = await query(`SELECT COUNT(*) as count FROM reservations r 
             JOIN reservation_statuses rs ON r.status_id = rs.status_id 
             WHERE rs.name = 'APPROVED'`);
    const pendingReservationsResult = await query(`SELECT COUNT(*) as count FROM reservations r 
             JOIN reservation_statuses rs ON r.status_id = rs.status_id 
             WHERE rs.name = 'PENDING'`);
    const rejectedReservationsResult = await query(`SELECT COUNT(*) as count FROM reservations r 
             JOIN reservation_statuses rs ON r.status_id = rs.status_id 
             WHERE rs.name = 'REJECTED'`);
    // Get statistics for the past 30 days
    const recentReservationsResult = await query(`SELECT COUNT(*) as count FROM reservations WHERE created_at >= NOW() - INTERVAL '30 days'`);
    // Get top facilities by reservation count
    const topFacilitiesResult = await query(`SELECT f.name as facility_name, COUNT(r.reservation_id) as reservation_count
             FROM reservations r
             JOIN reservation_statuses rs ON r.status_id = rs.status_id
...
export const getFacilityUtilization = catchAsync(async (req, res) => {
    // Keep existing query logic for now
    const facilityUtilizationResult = await query(`SELECT
            f.facility_id as id,
    ...
    const dailyUtilizationResult = await query(`SELECT
...
export const getUserActivity = catchAsync(async (req, res) => {
    const activeUsersResult = await query(`SELECT
            u.full_name as name,
            u.username,
            'N/A' as department,
         ...
         ORDER BY reservation_count DESC
         LIMIT 10`);
    const weeklyActivityResult = await query(`SELECT
