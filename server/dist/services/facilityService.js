...
export class FacilityService {
    ...
    async findAll(filters = {}) {
        ...
        if (filters.date && filters.startTime && filters.endTime) {
            ...
            if (conflictingIds.length > 0) {
                facilities = facilities.filter((f) => !conflictingIds.includes(f.facility_id));
            }
    ...
    async create(data) {
        ...
        if (!buildingId && data.building) {
            ...
            if (buildingEntity)
                buildingId = buildingEntity.building_id;
        }
        const facilityData = {
            name: data.name,
            type_id: typeId,
    ...
    async update(id, data) {
        ...
        if (data.type_id) {
            ...
            const typeEntity = await this.facilityTypeRepository.findByName(data.type);
            if (typeEntity)
                updateData.type_id = typeEntity.type_id;
        }
        ...
        if (data.building_id !== undefined) {
            ...
            const buildingEntity = await this.buildingRepository.findByNameOrCode(data.building);
            if (buildingEntity)
                updateData.building_id = buildingEntity.building_id;
        }
        if (data.room_number || data.roomNumber)
            updateData.room_number = data.room_number || data.roomNumber;
        if (data.capacity !== undefined)
            updateData.capacity = data.capacity;
        if (data.floor !== undefined)
            updateData.floor = data.floor;
        if (data.layout_description)
            updateData.layout_description = data.layout_description;
        if (data.description)
        ...
        // Maintenance fields
        if (data.maintenance_until !== undefined)
            updateData.maintenance_until = data.maintenance_until;
        if (data.maintenance_reason !== undefined)
            updateData.maintenance_reason = data.maintenance_reason;
