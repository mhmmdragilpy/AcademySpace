...
export class ReservationService {
    ...
    async update(id, userId, data) {
        ...
        if (existing.user_id !== userId)
            throw new AppError("Not authorized to edit this reservation", 403);
        ...
        if (timeChanged) {
            ...
            if (facilityId) {
                // Check Conflicts
                const conflicts = await this.reservationRepository.findConflicts(facilityId, startDateTime, endDateTime, id // Exclude self
                );
                if (conflicts.length > 0) {
                    throw new AppError("Time slot not available. Conflict with an existing reservation.", 409);
                }
