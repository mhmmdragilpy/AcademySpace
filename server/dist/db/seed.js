...
async function seedDatabase() {
    try {
        ...
        await query(`INSERT INTO system_tokens ("key", "value", "description") VALUES ($1, $2, $3), ($4, $5, $6)`, [
        ...
        for (const t of types) {
            try {
                const res = await query('INSERT INTO facility_types (name, description) VALUES ($1, $2) RETURNING type_id', [t.name, t.description]);
        ...
        for (const s of statuses) {
            const res = await query('INSERT INTO reservation_statuses (name) VALUES ($1) RETURNING status_id', [s]);
            statusMap.set(s, res.rows[0].status_id);
        }
        // 4. Insert Users
        logger.info('Creating users...');
        const adminPassword = await bcrypt.hash('rahasia', 10);
        await query(`INSERT INTO users (full_name, username, password_hash, role) 
             VALUES ($1, $2, $3, $4)`, ['Admin User', 'admin', adminPassword, 'admin']);
        // Optional standard user for testing
        const userPassword = await bcrypt.hash('user123', 10);
        await query(`INSERT INTO users (full_name, username, password_hash, role) 
             VALUES ($1, $2, $3, $4)`, ['John Doe', 'john_user', userPassword, 'user']);
        ...
        if (lines.length > 1) {
            ...
            for (const bName of buildingNames) {
                ...
                const res = await query('INSERT INTO buildings (name, code) VALUES ($1, $2) RETURNING building_id', [prettyName, bName]);
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                // Insert Facility
                await query(`INSERT INTO facilities (name, type_id, building_id, room_number, capacity, layout_description, photo_url, description) 
...
function generateDescription(name, type, capacity, buildingCode) {
    const amenities = [];
    let base = "";
    const lowerName = name.toLowerCase();
    // Building specific context (optional)
    let buildingContext = "";
    if (buildingCode === "TULT")
        buildingContext = "Located in the iconic TULT tower, offering stunning views and modern infrastructure.";
    else if (buildingCode === "GKU")
        buildingContext = "Situated in the General Course Building, a central hub for student activities.";
    else if (buildingCode === "SPORT_CENTER")
        buildingContext = "Part of the comprehensive Sport Center complex.";
