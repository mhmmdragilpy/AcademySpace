...
export const getAllReservations = async () => {
  try {
    ...
    if (err instanceof Error) {
      throw new AppError('Gagal mengambil semua reservasi: ' + err.message, 500);
    }
    throw new AppError('Gagal mengambil semua reservasi.', 500);
  }
};
...
export const updateReservationStatus = async (
  ...
  newStatusName: 'APPROVED' | 'REJECTED' | 'CANCELED', // Hanya boleh 3 ini
  adminId: number,
  comment: string | null
) => {
  ...
  try {
    ...
    const statusResult = await client.query(
      'SELECT status_id FROM reservation_statuses WHERE name = $1',
      [newStatusName]
    );

    if (statusResult.rows.length === 0) {
      throw new AppError(`Status '${newStatusName}' tidak valid.`, 400);
    }
    const newStatusId = statusResult.rows[0].status_id;

    // 2. Update tabel 'reservations' dengan status_id baru
    const updateResult = await client.query(
      'UPDATE reservations SET status_id = $1 WHERE reservation_id = $2 RETURNING reservation_id',
      [newStatusId, reservationId]
    );

    if (updateResult.rows.length === 0) {
      throw new AppError(`Reservasi dengan ID ${reservationId} tidak ditemukan.`, 404);
    }
    ...
    if (err instanceof Error) {
      throw new AppError('Gagal update status reservasi: ' + err.message, 500);
    }
    throw new AppError('Gagal update status reservasi.', 500);
  } finally {
