...
import type { Rating } from '../types/models/index.js';

export class RatingRepository extends BaseRepository<Rating> {
    protected tableName = 'ratings';
    protected primaryKey = 'rating_id'; // schema says 'id' likely, let's check or assume generic 'id' and map it
    ...
    async findByUserAndReservation(userId: number, reservationId: number) {
    ...
    async getAverageRating(facilityId: number) {
        const query = `
