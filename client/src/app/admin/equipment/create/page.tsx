/*
 * ===================================================================
 * FILE: client/src/app/admin/equipment/create/page.tsx (USING COMPONENTS)
 * ===================================================================
 */
'use client';
...
export default function CreateEquipmentPage() {
      ...
      try {
        ...
        if (axios.isAxiosError(err) && err.response) {
          if (err.response.status === 401 || err.response.status === 403) {
            ...
            setError('Gagal memuat tipe fasilitas.');
          }
        }
      } finally {
        setIsLoading(false);
  ...
  // 2. Handle Submit Form (POST /api/equipment)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!typeId || !name || !quantity || quantity < 1) {
        setError('Data tidak lengkap. Quantity minimal 1.');
      ...
      setSuccess(`Peralatan '${name}' berhasil dibuat!`);
      // Reset form (opsional)
      setName('');
      setLocation('');
      setQuantity(1);
      ...
      if (axios.isAxiosError(err) && err.response) {
        if (err.response.status === 409) {
          ...
          setError(err.response.data.message || 'Gagal menyimpan peralatan.');
        }
      } else {
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 className="text-4xl font-bold mb-6 text-gray-800">Tambah Peralatan Baru</h1>
